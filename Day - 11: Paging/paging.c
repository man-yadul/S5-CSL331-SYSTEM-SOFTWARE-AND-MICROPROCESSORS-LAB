...
void create()
{
    ...
    // Set number of frames iIf process size and frame size is exactly divisible
    if (proc_size % frame_size == 0)
        no_of_frames = proc_size / frame_size;
    // Process and frame size are not exactly divisible, set nof = psz / ssz + 1
    else
...
void delete()
{
    ...
    for (i = 0; i < proc_count; i++)
    {
        ...
        {
            flag = 1;
            // Deleting process by making its ID -1
            procs[i].pid = -1;
            break;
        }
    }
